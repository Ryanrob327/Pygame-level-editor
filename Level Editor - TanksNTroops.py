...
for x in range(tile_types):
    img = pygame.image.load(f'Tileset - TanksNTroops/{x}.png').convert_alpha()
...
while run:
    ...
    if save_button.draw(screen):
        with open(f'Levels - TanksNTroops/level{level}_data_TNT.csv', 'w', newline='') as csvfile:
    ...
    if load_button.draw(screen):
        x_scroll = 0
        y_scroll = 0
        with open(f'Levels - TanksNTroops/level{level}_data_TNT.csv', newline='') as csvfile:
